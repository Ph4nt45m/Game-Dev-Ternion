...
SettingsButton::SettingsButton(float x, float y)
    : Button(x, y),
...
bool SettingsButton::Initialise(Renderer& renderer)
{   
    // Load the button sprites
    m_buttonSpriteNormal = renderer.CreateSprite("..\\Sprites\\Menus\\Buttons\\settingsNormal.png");
    m_buttonSpriteHovered = renderer.CreateSprite("..\\Sprites\\Menus\\Buttons\\settingsHovered.png");
    m_buttonSpritePressed = renderer.CreateSprite("..\\Sprites\\Menus\\Buttons\\settingsPressed.png");
    ...
    m_buttonSpriteHighlight->SetX(m_x);
...
void SettingsButton::Update(float deltaTime, InputSystem& inputSystem)
{
    Button::Update(deltaTime, inputSystem);
    if (m_isReleased)
    {
        //SceneManager::GetInstance().ChangeScene(); // Once settings page has been done
    }
    else if (m_isHovered)
    {
        if (m_fBlueTint > 0.0f)
...
void SettingsButton::Draw(Renderer& renderer)
{
    if (m_isHeld)
    {
