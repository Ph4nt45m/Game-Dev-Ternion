...
ForestNight::ForestNight(b2World* world, Player* character)
...
bool
ForestNight::Initialise(Renderer& renderer)
{
	//Made by Rauen
...
void
ForestNight::Process(float deltaTime, InputSystem& inputSystem)
...
void
ForestNight::Draw(Renderer& renderer)
...
bool
ForestNight::SetEnemies(Renderer& renderer)
{
	...
	for (int i = 0; i < Game::GetInstance().difficulty; i++)
	{
		...
		m_vEnemies.push_back(m_pSkeleton);
...
bool
ForestNight::SetBGSprites(Renderer& renderer)
{
	// Day scene
	// Frame one
