...
IncreaseSoundButton::IncreaseSoundButton(float x, float y)
    : Button(x, y),
    m_buttonSpriteNormal(nullptr)
{
}

IncreaseSoundButton::~IncreaseSoundButton()
{
    delete m_buttonSpriteNormal;
    m_buttonSpriteNormal = nullptr;
}

bool IncreaseSoundButton::Initialise(Renderer& renderer)
{
...
void IncreaseSoundButton::Draw(Renderer& renderer)
{
    m_buttonSpriteNormal->Draw(renderer, false, false);
}

void IncreaseSoundButton::soundVolume()
{
    // Increase volume by 12
    int SoundVolume = SceneManager::GetInstance().getsoundEffectsVolume() + 13;
